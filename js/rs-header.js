...
function languageFunction() {
	const languageBlocks = document.querySelectorAll('.rs-header__language');

	if (languageBlocks.length > 0) {
		languageBlocks.forEach(languageBlock => {
			const menu = languageBlock.querySelector("ul");
			const items = menu.querySelectorAll("li");

			// Функция обновления позиции ::after
			function updateAfterPosition(target) {
				const { left } = target.getBoundingClientRect();
				const menuLeft = menu.getBoundingClientRect().left;
				menu.style.setProperty("--after-left", `${left - menuLeft}px`);
			}

			// Устанавливаем _active на загрузке
			const initialActive = menu.querySelector("li._active");
			if (initialActive) updateAfterPosition(initialActive);

			// Навешиваем события
			items.forEach((item) => {
				item.addEventListener("mouseenter", () => {
					updateAfterPosition(item);
					menu.querySelector("li._active")?.classList.remove("_active");
					item.classList.add("_active");
				});
				item.addEventListener("mouseleave", () => {
					updateAfterPosition(initialActive);
					menu.querySelector("li._active")?.classList.remove("_active");
					initialActive.classList.add("_active");
				});
			});
		});
	}
